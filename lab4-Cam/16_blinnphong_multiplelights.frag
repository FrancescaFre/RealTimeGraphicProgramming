/*
16_blinnphong_multiplelights.frag: as 13b_blinnphong.frag, but with multiple lights

N.B. 1)  "15_phong_multiplelights.vert" must be used as vertex shader
...
uniform sampler2D tex; //tex ->> texID    sampler: tipo di dati per leggere nella txture memory.
  ...
  vec3 N = normalize(vNormal);
    ...
    for(int i = 0; i < NR_LIGHTS; i++)
    {
      ...
      if(lambertian > 0.0) {
        ...
        color += vec3( Kd * lambertian * surfaceColor.rgb +
