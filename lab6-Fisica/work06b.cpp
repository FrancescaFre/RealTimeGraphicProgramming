...
int main()
{
  ...
  // we create the application's window
    ...
    btRigidBody* plane = bulletSimulation.createRigidBody(BOX, plane_pos, plane_size, plane_rot, 0.0, 0.3, 0.3);
...
//    vector<glm::vec3> positions;
    ...
    for(i = 0; i < num_side; i++ )
    {
        ...
        {
          cube.pos = glm::vec3((i - num_side)+3, 1.0f, (num_side - j));
          cube = bulletSimulation.createRigidBody(BOX, cube_pos, cube_size, cube_rot, 2.0, 0.3, 0.3);
  ...
  GLfloat maxSecPerFrame = 1.0f/60.0f; //il massimo deltatime per ricalcolare la rototraslation, lavora ad una frequena differente del RENDERING
  ...
  while(!glfwWindowShouldClose(window))
  {
      ...
      bulletSimulation.dynamicsWorld->stepSimulation((deltaTime<maxSecPerFrame ? deltaTime : maxSecPerFrame), 10); //10 è il nmero di substep ricorsivi che voglio prima di una soluzione
      ...
      planeModelMatrix = glm::mat(1.0); //serve per pulire il matrix, crea una matrice identità, cioè con gli 1 nella diagonale
      ...
      int num_cobjs = buletSimulation.dynamicsWorld->getNumCollisionObjects;
      ...
      for(i = 1; i <num_cobjs; i++ )
      {
          ...
          btCollisionObject* obj = bulletSimulation.dynamicsWorld->getCollisionObjectsArray()[i];
          btRigidBody* body = btRigidBody::upcast(obj);
          body->getMotionState()->getWorldTransform(transform); //restituisce la computaione eseguita
          transform.getOpneGLMatrix(matrix);

          objModelMatrix = glm::make_mat4(matrix) * glm::scale(objModelMatrix, obj_size); //conversione in un mat4
          //qui avviene il bind alla transofmr
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    btRidigBody* sphere;
    glm::mat4 unproject; //procedura del picking, per convertire quel punto della camera in un punto del dello schermo

    if(key == GLFW_KEY_SPACE && action == GLFW_PRESS)
    { //inizializzato al cameraPosition
      sphere =  bulletSimulation.createRigidBody(SPHERE, camera.Posizion, radius, rot, 1.0, 0.3, 0.3);
      //abbiamo bisogno della posizione del cursore da view port coordinate a normalize device coordinates
      shoot.x=(cursorX/screenWidth) * 2 - 1
      shoot.y= -(cursorY/screenHeight) * 2 + 1 //perchè il valore di y è verso il basso, l'origine è nell'angolo in alto a sinistra
      shoot.z=1.0f; //minimum depth
      shoot.w=1.0f;

      unproject = glm::inverse(projection*view);
      shoot = glm::normalize(unproject*shoot)*shootInitialSpeed; //inizializzo la velocità iniziale
