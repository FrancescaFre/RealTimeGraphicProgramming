...
enum RenderPasses{SHADOWMAP, RENDER};
...
int main()
{
    ...
    Shader shadow_shader("shadow.ver", "shadow.frag");
    SetupShaders();
    ...
    GLuin depthMap;
    GLuin depthMapFBO; //frame buffer object
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO); //serve per non far usare il frame buffer di default
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        //il primo passo di rendering, per settare tutto come
        //si crea una virtual camera per simulare l'area illuminata
        //---sono tutti parametri a cui fare tuning, è buona cosa metterli come variabili
        glm::mat4 lightProjection, lightView, lightSpaceMatrix;
        GLfloat near_plane = -10.0f, far_plane = 10.0f;
        GLfloat frustum_size = 5.0f;

        lightProjection = glm::ortho(-frustum_size, frustum_size, -frustum_size, frustum_size, near_plane, far_plane);

        lightView = glm::lookAt(lightDir0, glm::vec3(0.0), glm::vec3(0.0,1.0,0.0));
        ...
        //con questo è eeguito il primo passo di rendering
...
void RenderObjects(Shader &shader, Model &planeModel, Model &cubeModel, Model &sphereModel, Model &bunnyModel, GLint render_pass, GLuint depthMap)
{
    ...
    if(render_pass == RENDER){
      //texture 0: planes, texture1: object
...
void SetupShaders()
{
    ...
    Shader shader1("24_phong_tex_dirlight.vert", "24_ggx_tex_dirlight.frag");
