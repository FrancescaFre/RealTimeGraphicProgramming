...
vec4 GetLight(vec3 surfacePoint, vec3 cameraPosition, vec3 typeColor)
{
	...
	if(current_shader == 1){
		...
		finalColor = texture (iChannel0, R);
	}
	...
	if(current_shader == 2 || current_shader == 3){
		...
		vec3 R = reflect (V, normal);
		vec4 reflectedColor = texture(iChannel0, R);
		...
		refractedColor.r = texture(iChannel0, refractDir[0]).r;
		refractedColor.g = texture(iChannel0, refractDir[1]).g;
		refractedColor.b = texture(iChannel0, refractDir[2]).b;
